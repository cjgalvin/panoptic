...
#Casey - add RAW format
# The format to raw image.
_RAW_FORMAT = '%06d_raw'
    ...
    if FLAGS.also_save_raw_predictions:
      ...
      if train_id_to_eval_id is not None:
        crop_semantic_prediction = _convert_train_id_to_eval_id(
            ...
            train_id_to_eval_id)
      save_annotation.save_annotation(
          #Casey - changed file name to match other output
		  crop_semantic_prediction, raw_save_dir, _RAW_FORMAT % (image_id_offset + i),
          add_colormap=False)
...
def main(unused_argv):
  ...
  with g.as_default():
    ...
    while (FLAGS.max_number_of_iterations <= 0 or
      ...
      num_iters += 1
      last_checkpoint = FLAGS.checkpoint_dir#slim.evaluation.wait_for_new_checkpoint(
          #FLAGS.checkpoint_dir, last_checkpoint=FLAGS.checkpoint_dir)
      start = time.time()
      tf.logging.info(
